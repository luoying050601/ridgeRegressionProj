...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None,
        ...
        if attention_mask is not None:
            ...
            # attention_scores torch.Size([1, 16, 73, 73])
        ...
        # attention_probs = attention_probs * self.time_weighting[:, :T, :T]  # 只需增加这句
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask,head_mask,
        ...
        # step 1.1: 如果是decoder, 就作cross-attention，此时step1.0的输出即为decoder侧的序列的self-attention结果，
        # 并作为step1.1的输入；step 1.1的输出为decoder侧的cross-attention结果, cross_attention实例也是BertAttention
        ...
        return outputs
        # # BertLayer分为两个操作，BertAttention和BertIntermediate。
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # to the first token.
        first_token_tensor = hidden_states[:, 0] #输出cls位置的值
